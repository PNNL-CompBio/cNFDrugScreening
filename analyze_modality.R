# analyze_modality.R — original-style behavior, no debug, no metric normalization
...
# ------------------------------------------------------------
# leapR export helpers (target drug selection + bar plot export)
# ------------------------------------------------------------

# 0) Safe label (define only if not present)
if (!exists("safelabel", mode = "function")) {
  safelabel <- function(x) gsub("[^A-Za-z0-9_.-]", "_", x)
}

# 1) Choose target drugs from summarize_drugs(...) output
#    - n_eff: top-N most efficacious (lowest meanResponse)
#    - n_var: top-N most variable (highest variability)
#    - include_regex: case-insensitive pattern to force-include matches (e.g., "onalespib")
choose_target_drugs <- function(
    drug_summary,
    n_eff = 2,
    n_var = 2,
    include_regex = "onalespib",
    ignore_case = TRUE
) {
  stopifnot(all(c("improve_drug_id","meanResponse","variability") %in% names(drug_summary)))

  top_eff <- drug_summary %>%
    arrange(.data$meanResponse) %>%
    slice_head(n = n_eff) %>%
    pull(.data$improve_drug_id)

  top_var <- drug_summary %>%
    arrange(desc(.data$variability)) %>%
    slice_head(n = n_var) %>%
    pull(.data$improve_drug_id)

  forced <- character(0)
  if (!is.null(include_regex) && nzchar(include_regex)) {
    forced <- drug_summary %>%
      dplyr::filter(grepl(include_regex, .data$improve_drug_id, ignore.case = ignore_case)) %>%
      pull(.data$improve_drug_id)
  }

  unique(c(top_eff, top_var, forced))
}

# 2) Save leapR TOP/BOTTOM plots for one drug & one modality
#    - Assumes res_obj[[drug]] has $top (Resistant) and $bottom (Sensitive) tables
save_leapr_bar_for_drug <- function(drug, res_obj, omic_label, out_dir, top_n = 15) {
  if (is.null(res_obj) || !(drug %in% names(res_obj))) {
    message("[", omic_label, "] Drug ", drug, " not found in res object; skipping.")
    return(invisible(NULL))
  }

  two <- res_obj[[drug]]
  if (is.null(two)) {
    message("[", omic_label, "] Drug ", drug, " has NULL entry; skipping.")
    return(invisible(NULL))
  }
  ...
  if (!is.null(two$top)) {
    ...
    if (!is.null(p_top)) {
      ggplot2::ggsave(
        filename = file.path(out_dir, paste0("leapr_", safelabel(omic_label), "_", safelabel(drug), "_TOP.pdf")),
        plot     = p_top,
        width    = 7, height = 5, device = grDevices::cairo_pdf
      )
    }
  } else {
    message("[", omic_label, "] Drug ", drug, " has no TOP result.")
  }
  ...
  if (!is.null(two$bottom)) {
    ...
    if (!is.null(p_bot)) {
      ggplot2::ggsave(
        filename = file.path(out_dir, paste0("leapr_", safelabel(omic_label), "_", safelabel(drug), "_BOTTOM.pdf")),
        plot     = p_bot,
        width    = 7, height = 5, device = grDevices::cairo_pdf
      )
    }
  } else {
    message("[", omic_label, "] Drug ", drug, " has no BOTTOM result.")
  }

  invisible(NULL)
}

export_leapr_bars_for_modalities <- function(
    target_drugs,
    results_by_modality,
    out_dir = "figs",
    top_n = 15
  ...
  for (drug in target_drugs) {
    message("=== ", drug, " ===")
    for (omic_label in names(results_by_modality)) {
      save_leapr_bar_for_drug(
        drug       = drug,
        res_obj    = results_by_modality[[omic_label]],
...
# 4) One-shot wrapper starting from a drug_summary + three res objects
run_leapr_exports <- function(
    drug_summary,
    res_bio_rna,
    res_bio_global,
    res_bio_phospho,
  ...
  target_drugs <- choose_target_drugs(
    drug_summary,
    n_eff = n_eff,
    n_var = n_var,
  ...
  message("Target drugs: ", paste(target_drugs, collapse = ", "))

  results_by_modality <- list(
    RNA     = res_bio_rna,
    Global  = res_bio_global,
    Phospho = res_bio_phospho
  )

  export_leapr_bars_for_modalities(
    target_drugs = target_drugs,
    results_by_modality = results_by_modality,
    out_dir = out_dir,
    top_n = top_n
  )
  invisible(target_drugs)
}
