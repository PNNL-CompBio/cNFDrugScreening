"""
Drug response curve fitting model. Taken from:
https://raw.githubusercontent.com/PNNL-CompBio/coderdata/refs/heads/main/build/utils/fit_curve.py
"""
#! /usr/bin/env python3
...
def format_coderd_schema(fname):
    ...
    new_df = pd.melt(df2,
                     id_vars=['source', 'improve_sample_id', 'improve_drug_id',
                              'study', 'time', 'time_unit'],
                     value_vars=['fit_auc', 'fit_ic50', 'fit_ec50', 'fit_r2',
                                 'fit_ec50se', 'fit_einf', 'fit_hs', 'aac',
                                 'auc', 'dss'],
...
HS_BOUNDS_NEG = ([0, -5, -1], [1, 3,
                               1])  ## made hill slope forced to be negative  ##20241017 updated to shift EC50 range
...
def fit_exp(df_exp, title=None, dmin=None, dmax=None, save=False):
    ...
    plt.plot(xx, yy * 100, 'r-',
             label='fit: einf=%.3f, ec50=%.3f, hs=%.3f' % tuple(popt))
...
def compute_fit_metrics(xdata, ydata, popt, pcov, d1=4,
                        d2=10):  # d1 = -5, d2=3):
    '''
    ...
    '''
    if popt is None:
        cols = ['fit_auc', 'fit_ic50', 'fit_ec50', 'fit_ec50se', 'fit_r2',
                'fit_einf', 'fit_hs', 'aac', 'auc',
                'dss']  # 'auc ic50 ec50 ec50se R2fit rinf hs aac1 auc1 dss1'.split(' ')
    ...
    dss1 = (0.9 * (ic10x - xmin) - int10x) / (
                0.9 * (xmax - xmin)) if xmin < ic10x else 0
    # this auc has fixed doses, so can be (in theory) standardized across datasets
    auc = (response_integral(d2, *popt) - response_integral(d1, *popt)) / (
                d2 - d1)
    ...
    dss1 = (auc1 - 0.1 * (ic10x - xmin)) / (
                0.9 * (xmax - xmin)) if xmax > ic50 else 0
    dss2 = dss1 / (1 - einf)  ##made this dss2 doesn't change much
    metrics = pd.Series(
        {'fit_auc': auc, 'fit_ic50': ic50, 'fit_ec50': ec50, 'fit_einf': einf,
...
def response_curve_fit(xdata, ydata, bounds=HS_BOUNDS_M):
    ...
    while popt is None and nfev < 10000:
        # print(nfev)
        try:
            popt, pcov = curve_fit(response_curve, xdata, ydata, bounds=bounds,
                                   max_nfev=nfev)
...
def fit_model(
    ...
    df_all = pd.read_table(input_path)
    if debug:
    ...
    if output_path is None:
        output_path = 'combined_single_response_agg'

    process_df_part(
        df_all,
        output_path,
    )
    format_coderd_schema(output_path + '.0')
...
def main():
    ...
    parser.add_argument('--beataml', action='store_true',
                        help='Include this if for BeatAML')
    ...
    process_df_part(df_all, fname,
                    beataml=args.beataml)  # , start=args.start, count=args.count)
